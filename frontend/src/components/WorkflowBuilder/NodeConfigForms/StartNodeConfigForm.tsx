            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  type="button"
                  onClick={() => {
                    // Use VITE_API_URL if set, otherwise use development default or current origin
              ...
              <div className="bg-white border border-blue-300 rounded p-2">
                <code className="text-xs font-mono text-blue-900 break-all">
                  {(import.meta.env.VITE_API_URL !== undefined 
                    ? import.meta.env.VITE_API_URL 
                    : (import.meta.env.DEV ? 'http://localhost:5000' : window.location.origin))}/api/webhook/{workflowId}
                </code>
              </div>
            </div>
